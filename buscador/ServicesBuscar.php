 ...
 <link href="../vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
   ...
   if (isset($_POST['tipo'])) {
     switch ($_POST['tipo']) {
       case 'Todos':
       $sqlBuscar = "Select * from hospedaje inner join Subasta on (Subasta.idHospedaje = hospedaje.idHospedaje) inner join fechasDisponibles on (Subasta.idFecha = fechasDisponibles.idFecha) inner join Direccion ON (Direccion.idDireccion = hospedaje.idDireccion) inner join provincias ON (Direccion.Provincia = provincias.id) where Subasta.estado = 1 OR Subasta.estado = 3";
       $sqlBuscarHotsales = "Select * from hospedaje inner join hotsale on (hotsale.idHospedaje = hospedaje.idHospedaje) inner join fechasDisponibles on (hotsale.idFecha = fechasDisponibles.idFecha) inner join Direccion ON (Direccion.idDireccion = hospedaje.idDireccion) inner join provincias ON (Direccion.Provincia = provincias.id) where hotsale.estado = 1";
         break;
       case 'Subasta':
       $sqlBuscar = "Select * from hospedaje inner join Subasta on (Subasta.idHospedaje = hospedaje.idHospedaje) inner join fechasDisponibles on (Subasta.idFecha = fechasDisponibles.idFecha) inner join Direccion ON (Direccion.idDireccion = hospedaje.idDireccion) inner join provincias ON (Direccion.Provincia = provincias.id) where Subasta.estado = 1 OR Subasta.estado = 3";
       break;
   ...
   if (isset($_GET['tipo'])) {
     switch ($_GET['tipo']) {
       case 'Todos':
       $sqlBuscar = "Select * from Subasta inner join hospedaje on (Subasta.idHospedaje = hospedaje.idHospedaje) inner join fechasDisponibles on (Subasta.idFecha = fechasDisponibles.idFecha) inner join Direccion ON (Direccion.idDireccion = hospedaje.idDireccion) inner join provincias ON (Direccion.Provincia = provincias.id) where Subasta.estado = 1 OR Subasta.estado = 3";
       $sqlBuscarHotsales = "Select * from hospedaje inner join hotsale on (hotsale.idHospedaje = hospedaje.idHospedaje) inner join fechasDisponibles on (hotsale.idFecha = fechasDisponibles.idFecha) inner join Direccion ON (Direccion.idDireccion = hospedaje.idDireccion) inner join provincias ON (Direccion.Provincia = provincias.id) where hotsale.estado = 1";
         break;
       case 'Subasta':
       $sqlBuscar = "Select * from hospedaje inner join Subasta on (Subasta.idHospedaje = hospedaje.idHospedaje) inner join fechasDisponibles on (Subasta.idFecha = fechasDisponibles.idFecha) inner join Direccion ON (Direccion.idDireccion = hospedaje.idDireccion) inner join provincias ON (Direccion.Provincia = provincias.id) where Subasta.estado = 1 OR Subasta.estado = 3";
   ...
   if (isset($_POST['buscadorNombre'])){
    if (strlen($_POST['buscadorNombre']) > 0){
      if(isset($sqlBuscar)){
        $sqlBuscar .= " AND (hospedaje.titulo LIKE '%".$_POST['buscadorNombre']."%' OR Direccion.Direccion LIKE '%".$_POST['buscadorNombre']."%' OR Direccion.ciudad LIKE '%".$_POST['buscadorNombre']."%' OR provincias.provincia LIKE '%".$_POST['buscadorNombre']."%' )";
      }
      if (isset($sqlBuscarHotsales)){
        $sqlBuscarHotsales .= " AND (hospedaje.titulo LIKE '%".$_POST['buscadorNombre']."%' OR Direccion.Direccion LIKE '%".$_POST['buscadorNombre']."%' OR Direccion.ciudad LIKE '%".$_POST['buscadorNombre']."%' OR provincias.provincia LIKE '%".$_POST['buscadorNombre']."%' )";
      }
    }
  }
 ...
 <?php
   ...
   <?php } ?>
       ...
       <div id="condenido">
         ...
         <?php
         if  ( isset($sqlBuscar) && isset($sqlBuscarHotsales) && mysqli_num_rows($resultadoHotsale) == 0 && mysqli_num_rows($resultadoSubasta) == 0 ) {
           echo "<h2>No hay hospedajes disponibles para su busqueda</h2>";
         }
         if (isset($sqlBuscar)){
           if(mysqli_num_rows($resultadoSubasta) == 0 && !(isset($sqlBuscarHotsales))){
             echo "<h2>No hay hospedajes disponibles para su busqueda</h2>";
           }
         }
         $contador=1;
            ...
            <div id="hotsale">
              ...
              <div id="adquirir">
                ...
                <?php  if (isset($_SESSION['idUsuario'])){
                  ...
                  if (isset($_GET['tipo'])){
                    ?>
                    <a href="../subastas/adquirir.php?return=<?php echo $actual_link;?>&id=<?php echo $rowHotsale['idHotsale'];?>" class="btn btn-block btn-lg btn-warning">ADQUIRIR!</a><?php
                  } else {
                  if (isset($_POST['tipo'])){
                  ?>
                  <a href="../subastas/adquirir.php?return=<?php echo $actual_link;?>?tipo=<?php echo $_POST['tipo'];?>&id=<?php echo $rowHotsale['idHotsale'];?>" class="btn btn-block btn-lg btn-warning">ADQUIRIR!</a>
                  <?php
                  }
                }
                }
        ...
        if (isset($sqlBuscar)) {
           while ($rowsSubasta = mysqli_fetch_array($resultadoSubasta)){
             ...
             $date = str_replace('00:00 AM', '', $date1 );
           ?>
          <?php switch ($rowsSubasta['estado']) {

            case 1:
            $dateEnd = $rowsSubasta['fechaFin'];
            $dateEnd = str_replace('-', '/', $dateEnd );
            $dateEnd = str_replace(' 00:00:00', ' 00:00 AM', $dateEnd );
            $dateEnd = str_replace('00:00 AM', '', $dateEnd );
            ?>
            <script>

            var end<?php echo $contador; ?> = new Date('<?php echo $dateEnd; ?>');
            ...
            function showRemaining<?php echo $contador; ?>() {
                ...
                if (distance<?php echo $contador; ?> < 0) {
                    ...
                    clearInterval(timer<?php echo $contador; ?>);
                    document.getElementById('countdown<?php echo $contador; ?>').innerHTML = 'SUBASTA TERMINADA!';
                    document.getElementById('countdown<?php echo $contador; ?>').innerHTML +=' <form name="myForm" id="myForm" action="terminarSubasta.php" method="POST"> <input type="hidden" name="id" value="<?php echo $rowsSubasta['idSubasta']; ?>" /></form>';
            ...
            <div id="subasta">
              ...
              <div id="info">
                ...
                <span>La subasta termina el: <?php echo $dateEnd;?></span>
             </div>
            <div id="adquirir" style="margin-top: -200px;">
              <div id="countdown<?php echo $contador; ?>">
                  Quedan
                <span class="dias" id="dias<?php echo $contador; ?>">
                </span>
            ...
            $dateEnd = str_replace('-', '/', $dateEnd );
            $dateEnd = str_replace(' 00:00:00', ' 00:00 AM', $dateEnd );
            $dateEnd1 = str_replace('00:00 AM', '', $dateEnd );
            ?>
            <script>
            ...
            function showRemaining<?php echo $contador; ?>() {
                ...
                if (distance<?php echo $contador; ?> < 0) {
                    ...
                    clearInterval(timer<?php echo $contador; ?>);
                    document.getElementById('countdown<?php echo $contador; ?>').innerHTML = 'SUBASTA INICIADA!';
                    document.getElementById('countdown<?php echo $contador; ?>').innerHTML +=' <form name="myForm" id="myForm" action="empezar.php" method="POST"> <input type="hidden" name="id" value="<?php echo $rowsSubasta['idSubasta']; ?>" /></form>';
                    var auto_refresh = setInterval(
                    function()
                    {
                    submitform();
            ...
            <div id="subasta">
              ...
              <div id="info">
                <span><h4>Para usar la semana del </h4><h4><?php echo $date;?></h4></span>
                <span><h4>Precio Base  $<?php echo $rowsSubasta['precio'];?></h4></span>
                <span>La subasta comienza el: <?php echo $dateEnd1;?></span>
             </div>
            <div id="adquirir" style="margin-top: -250px;">
              <div id="countdown<?php echo $contador; ?>">
                Quedan
                <span class="dias" id="dias<?php echo $contador; ?>">
                </span>
                ...
                para inscribirse y recibir la notificacion cuando comience esta Subasta
              </div>
              <?php
              if (isset($_SESSION['idUsuario'])){
              if (isset($_GET['tipo'])){
                if($_SESSION['tipo'] == 3){
                  ?>
                  <a href="../subastas/adquirir.php?inscripcion=1&return=<?php echo $actual_link;?>&id=<?php echo $rowsSubasta['idSubasta'];?>" class="btn btn-block btn-lg btn-warning">ADQURIR!</a>
                  <?php
                }
                ?>
                <a href="../subastas/inscribirme.php?return=<?php echo $actual_link;?>&id=<?php echo $rowsSubasta['idSubasta'];?>" class="btn btn-block btn-lg btn-primary">RECIBIR!</a><?php
              } else {
              if (isset($_POST['tipo'])){
                if($_SESSION['tipo'] == 3){
                  ?>
              ...
              ?>
              <a href="../subastas/inscribirme.php?return=<?php echo $actual_link;?>?tipo=<?php echo $_POST['tipo'];?>&id=<?php echo $rowsSubasta['idSubasta'];?>" class="btn btn-block btn-lg btn-success">RECIBIR!</a>
              <?php
              }
              }
            }?>
