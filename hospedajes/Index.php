  ...
  <body>
    ...
    <div class="container">
      <a class="navbar-brand" href="../index.php"><img class="logo" title="Home Switch Home" alt="Home Switch Home" src="../Logos/HSH-Complete.svg" width="200px"></a>
            ...
            <?php
              ...
              if (isset($_SESSION['idUsuario'])){
                if($_SESSION['tipo']==1){?>
                  <a class="btn btn-primary" href="subasta/verSolicitudesHotsale.php">Solicitudes a Hotsale</a>
                  <a class="btn btn-warning" href="subasta/verTodosHotsales.php">Ver todos los Hotsales Publicados</a>
      ...
      <div id="condenido">
        ...
        <a class="btn btn-success" style="margin-top:-90px" href="ABM/Alta.php">Alta Hospedajes</a>
        <table class="table table">
         <thead>
          ...
          while ($rows = mysqli_fetch_array($resultado)){
            $sqlHosale = "SELECT idHospedaje FROM hotsale WHERE idHospedaje =".$rows['idHospedaje']." AND estado = 1";
            $resultadoHotsale = mysqli_query($con,$sqlHosale);
            if (mysqli_num_rows($resultadoHotsale)) {
              $enHotsale = true;
              }
              else {
                $enHotsale = false;
              }
              $sqlCantidadPersonas = "Select idHospedaje from Subasta WHERE idHospedaje = ".$rows['idHospedaje']." AND estado = 1 OR estado = 3";
              ...
              <tr>
                ...
                <?php
                if($_SESSION['tipo']==1){?>
                  <td class="table-primary"><a class="btn btn-warning" href='subasta/crearSubasta.php?id=<?php echo $rows['idHospedaje'];?>'>Ver estado de subastas/inscripciones</a></td>
                  <?php if (($enSubasta) || ($enHotsale)){ ?>
                    <td class="table-primary"><small class="form-text text-muted">No se puede elimiar el hospedaje porque posee subastas activas o se encuentra en Hotsale</small></td>
                  ...
                  if($_SESSION['tipo']==3){?>
                  <td class="table-primary"><a class="btn btn-danger" href='../premium/compraDirecta.php?id=<?php echo $rows['idHospedaje'];?>'>Compra YA!</a></td><?php
                  }?>
              </tr>
              <script>
              function confirmation() {
      return confirm('Seguro desea eliminar este Hospedaje');
    }
              </script>
          <?php
          }
