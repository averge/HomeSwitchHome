...
if (isset($_GET['id'])){
  ...
  if (isset($_GET['inscripcion'])){
    ...
    $sqlSubasta="SELECT * FROM hotsale WHERE idHotsale=".$_GET['id'];
    $resultadoSubasta = mysqli_query($con,$sqlSubasta);
    $rowsSubasta = mysqli_fetch_assoc($resultadoSubasta);
  }
     ...
     <div class="content-wrapper">
      ...
      ?>
          ...
          <div class="product-desc"><h4>Ubicacion: <?php echo utf8_encode($rowsDireccionCompleta['Direccion'].', '.$rowsDireccionCompleta['ciudad'].', '.$rowsDireccionCompleta['provincia'].', Argentina'); ?></h4></div>
            ...
            <form action="ServiceAdquirir.php" method="post">
              <input type="hidden" name="id" value="<?php echo $_GET['id'];?>">
              <input type="hidden" name="idH" value="<?php echo $rowsSubasta['idHospedaje'];?>">
              <input type="hidden" name="idF" value="<?php echo $rowsSubasta['idFecha'];?>">
              <div class="col-auto">
                <div class="input-group mb-2">
                  <div class="input-group-prepend">
                    ...
                    <?php if ($_SESSION['tipo']==2) {
                      ?>
                      <input type="text" value="DEBE SER PREMIUM" readonly/>
                      <input type="button" style="margin-left:8px; border-radius:8px;font-size:18pt"name="ofertar" value="ADQUIRIR" class="btn btn-secondary">
                    </form>
                      <?php
                    } else { if ($_SESSION['tipo']==1){
                    ?>
                    <input type="text" value="USUARIO ADMINISTRADOR NO PUEDE ADQUIRIR" readonly/>
                    <input type="button" style="margin-left:8px; border-radius:8px;font-size:18pt"name="ofertar" value="ADQUIRIR" class="btn btn-secondary">
                    <?php
                    } else { ?>
                      <input type="text" value="<?php echo $rowsSubasta['precio'] ?>" readonly/>
                      <?php
                      if (isset($_GET['inscripcion'])){
                        ?>
                       ...
                       ?>
                      <input type="submit" style="margin-left:8px; border-radius:8px;font-size:18pt"name="ofertar" value="ADQUIRIR" class="btn btn-success">
                      <?php
                    }
                  }?>
        </div>
